...
for powerii in plist1
    ...
    mkpath(projectdir("exp","prm",expii,conii))
    for imember = 1 : 15
        oprm  = projectdir("exp","prm",expii,"damping001","member01.prm")
        nprm  = projectdir("exp","prm",expii,conii,"member$(@sprintf("%02d",imember)).prm")
        open(tprm,"w") do fprm
            open(oprm,"r") do rprm
                s = read(rprm,String)
