...
for powerii in plist1
    ...
    mkpath(projectdir("exp","prm","Ensemble",expii,conii))
    for imember = 1 : 3
        oprm  = projectdir("exp","prm","Ensemble",expii,"damping001","member01.prm")
        nprm  = projectdir("exp","prm","Ensemble",expii,conii,"member$(@sprintf("%02d",imember)).prm")
        open(tprm,"w") do fprm
            open(oprm,"r") do rprm
                s = read(rprm,String)
