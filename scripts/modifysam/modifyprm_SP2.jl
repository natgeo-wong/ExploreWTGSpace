...
for powerii in pvec
    ...
    mkpath(projectdir("exp","prm","SP2",expname,conii))
    for imember = 1 : 15
        mstr = @sprintf("%02d",imember)
        oprm = projectdir("run","modifysam","prmtemplates","SP2_$(expname).prm")
        nprm = projectdir("exp","prm","SP2",expname,conii,"member$(mstr).prm")
        open(tprm,"w") do fprm
            open(oprm,"r") do rprm
                s = read(rprm,String)
                s = replace(s,"[xx]" => mstr)
        ...
        @info "Creating new prm file for SP2 $expname $conii ensemble member $imember"
    end
end
