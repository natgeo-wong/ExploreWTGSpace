...
begin
	...
	fts.savefig(plotsdir(
		"02c-bifurcation-$(expname)-$(schname).png"),
		transparent=false,dpi=400
	)
	load(plotsdir("02c-bifurcation-$(expname)-$(schname).png"))
end
...
begin
	if isone(createimage)
		pplt.close()
		f3D,a3D = pplt.subplots(ncols=6,nrows=2,aspect=0.4,axwidth=1,sharex=0,wspace=1.5)
		clc = zeros(64)
		tab = zeros(64)
		swh = zeros(64)
		...
		for ic in 1 : nconDGW
			...
			while imem < 15; imem += 1
				fnc = outstatname("DGW",expname,icon,false,true,imem)
				if isfile(fnc)
					_,p,t = retrievedims_fnc(fnc); t = t .- 80
		...
		a3D[10].format(xlim=(-2,2),xlabel=L"$w_{WTG}$ / km hr$^{-1}$",xscale="symlog",
		xscale_kw=Dict("linthresh"=>0.01),ultitle="(d)")
		...
		f3D.savefig(plotsdir(
			"02c-vertprofiles-$(expname)-$(schname).png"),
			transparent=false,dpi=200
		)
	end
	load(plotsdir("02c-vertprofiles-$(expname)-$(schname).png"))
end
